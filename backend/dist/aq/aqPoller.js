...
function shouldExcludeQueue(owner, queueName) {
    const cfg = getConfig();
...
async function cleanExcludedQueuesFromDB() {
    ...
    return new Promise((resolve) => {
        ...
        db.run(`DELETE FROM queue_snapshots WHERE ${whereClause}`, function (err) {
            db.close();
            if (err) {
...
async function pollDatabase(dbId) {
    ...
    `;
        ...
        const filteredQueues = queuesResult.rows.filter(q => {
        ...
        const snapshots = await Promise.all(filteredQueues.map(async (queueRow) => {
            let queueConn;
            try {
          ...
          `;
                const countResult = await queueConn.execute(countSql, { qname: queueRow.QUEUE_NAME }, { outFormat: oracledb.OUT_FORMAT_OBJECT });
                const lastDeqResult = await queueConn.execute(lastDeqSql, { qname: queueRow.QUEUE_NAME }, { outFormat: oracledb.OUT_FORMAT_OBJECT });
                const countRow = countResult.rows && countResult.rows.length > 0 ? countResult.rows[0] : null;
                const messageCount = countRow ? Number(countRow.MSG_COUNT) || 0 : 0;
                const deqRow = lastDeqResult.rows && lastDeqResult.rows.length > 0 ? lastDeqResult.rows[0] : null;
 ...
 */
async function checkBlockedQueuesAndAlert(snapshots, dbId) {
