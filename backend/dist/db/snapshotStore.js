    ...
    `, (err) => {
        ...
        `, (indexErr) => {
                    if (indexErr) {
                        ...
                        db.all(`PRAGMA table_info(queue_snapshots)`, (pragmaErr, columns) => {
                            if (pragmaErr) {
                                db.close();
...
export async function saveSnapshot(snapshot) {
    return new Promise((resolve, reject) => {
        const db = openDatabase();
        db.run(`INSERT INTO queue_snapshots (queue_name, db_id, message_count, polled_at, last_dequeued) 
       VALUES (?, ?, ?, ?, ?)`, [snapshot.queue_name, snapshot.db_id, snapshot.message_count, snapshot.polled_at, snapshot.last_dequeued || null], (err) => {
            db.close();
            if (err) {
...
export async function saveSnapshotBatch(snapshots) {
    ...
    return new Promise((resolve, reject) => {
        ...
        db.serialize(() => {
            ...
            const stmt = db.prepare(`INSERT INTO queue_snapshots (queue_name, db_id, message_count, polled_at, last_dequeued) 
         VALUES (?, ?, ?, ?, ?)`);
            let errorOccurred = false;
            for (const snapshot of snapshots) {
                stmt.run([snapshot.queue_name, snapshot.db_id, snapshot.message_count, snapshot.polled_at, snapshot.last_dequeued || null], (err) => {
...
function calculateGroup(queueName) {
...
export async function getLatestSnapshots(dbId) {
    return new Promise((resolve, reject) => {
        const db = openDatabase();
    ...
    `, params.length > 0 ? [...params, ...params] : [], (err, rows) => {
            ...
            if (err) {
                ...
                const snapshotsWithGroup = rows.map(snapshot => ({
                    ...snapshot,
...
export async function deleteQueueSnapshots(queueNames) {
    return new Promise((resolve, reject) => {
        const db = openDatabase();
        const placeholders = queueNames.map(() => '?').join(', ');
        db.run(`DELETE FROM queue_snapshots WHERE queue_name IN (${placeholders})`, queueNames, function (err) {
            db.close();
            if (err) {
...
export async function cleanOldSnapshots(daysToKeep = 30) {
    return new Promise((resolve, reject) => {
        const db = new sqlite3.Database(dbFile);
        const cutoffDate = new Date();
        cutoffDate.setDate(cutoffDate.getDate() - daysToKeep);
        db.run(`DELETE FROM queue_snapshots WHERE polled_at < ?`, [cutoffDate.toISOString()], function (err) {
            db.close();
            if (err) {
