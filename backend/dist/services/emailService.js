...
const runtimeEmailEnabledPerDb = new Map();
/**
 * Check if email is enabled for a specific database
 */
export function isEmailEnabledForDb(dbId) {
 ...
 */
export function setEmailEnabled(enabled, dbId) {
    if (dbId) {
 ...
 */
export function getEmailStatus(dbId) {
    const config = getConfig();
 ...
 */
function isEmailEnabledForEnvironment() {
    const config = getConfig();
    const emailConfig = config.notifications?.email;
    if (!emailConfig || !emailConfig.enabled) {
        return false;
 ...
 */
function canSendAlert(queueName) {
 ...
 */
export async function sendBlockedQueueAlert(alert) {
    try {
