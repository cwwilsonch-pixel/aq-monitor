...
interface QueueDiscoveryConfig {
  excludeQueues?: string[];
  excludeQueueTables?: string[];
  includePattern?: string;
  groupByPrefix?: boolean;
  enabled?: boolean;
  includeSystemUsers?: string[];
}
...
export interface Config {
  ...
  queueDiscovery?: QueueDiscoveryConfig;
  auth?: {
    ldap?: any;
    local?: any;
  };
...
export function loadConfig(): Config {
  ...
  if ((config as any).notifications?.email) {
    const emailConfig = (config as any).notifications.email;
