...
async function getDbIdForQueue(queueName: string): Promise<string | null> {
  return new Promise((resolve, reject) => {
    const db = new sqlite3.Database(dbFile);
    
    db.get(
      `SELECT db_id FROM queue_snapshots 
       WHERE queue_name = ? 
...
export async function getQueueMetrics(queueName: string, dbId?: string) {
  ...
  try {
    ...
    const queueInfoResult = await conn.execute(
      queueInfoSql,
      { owner, queue },
    ...
    const queueTable = (queueInfoResult.rows[0] as any).QUEUE_TABLE;
    ...
    try {
      ...
      const readyCountResult = await conn.execute(
        readyCountSql,
        { queue },
        { outFormat: oracledb.OUT_FORMAT_OBJECT }
      );
      ...
      readyCount = readyCountResult.rows && readyCountResult.rows.length > 0
        ? Number((readyCountResult.rows[0] as any).READY_COUNT) || 0
        : 0;
    } catch (err: any) {
    ...
    const totalMessages = totalResult.rows && totalResult.rows.length > 0
      ? Number((totalResult.rows[0] as any).TOTAL) || 0
      : 0;
    ...
    const peakResult = await conn.execute(
      peakHourSql,
      { queue },
      { outFormat: oracledb.OUT_FORMAT_OBJECT }
    );
    ...
    if (peakResult.rows && peakResult.rows.length > 0) {
      const row = peakResult.rows[0] as any;
    ...
    const lastProcessedResult = await conn.execute(
      lastProcessedSql,
      { queue },
      { outFormat: oracledb.OUT_FORMAT_OBJECT }
    );
    ...
    if (lastProcessedResult.rows && lastProcessedResult.rows.length > 0) {
      const row = lastProcessedResult.rows[0] as any;
    ...
    try {
      const retryStatsResult = await conn.execute(
        retryStatsSql,
        { queue },
        { outFormat: oracledb.OUT_FORMAT_OBJECT }
      );
      ...
      if (retryStatsResult.rows && retryStatsResult.rows.length > 0) {
        const row = retryStatsResult.rows[0] as any;
    ...
    const dailyStatsResult = await conn.execute(
      dailyStatsSql,
      { queue },
      { outFormat: oracledb.OUT_FORMAT_OBJECT }
    );
    
    const dailyStats = (dailyStatsResult.rows || []).map((row: any) => ({
      date: row.DT,
...
export async function getHistoricData(from: Date, to: Date) {
  const dbFile = process.env.SQLITE_FILE || './data/aq-monitor.db';
