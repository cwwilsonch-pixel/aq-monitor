      ...
      client.search(ldapConfig.searchBase, opts, (searchErr, searchRes) => {
        ...
        searchRes.on('searchEntry', (entry) => {
          ...
          if (ldapConfig.groupAttribute) {
            const groupAttr = entry.attributes.find(attr => 
              attr.type.toLowerCase() === ldapConfig.groupAttribute.toLowerCase()
            );
            
            // Group attribute found, processing values
            
            // Try different ways to get the values
            const values = (groupAttr as any)?._vals || groupAttr?.vals || groupAttr?.values;
            
            if (values && Array.isArray(values) && values.length > 0) {
              userGroups = values.map((dn: any) => {
        ...
        searchRes.on('end', () => {
          ...
          const hasRequiredGroup = allowedGroups.length === 0 || 
            userGroups.some(group => allowedGroups.some(allowed => 
