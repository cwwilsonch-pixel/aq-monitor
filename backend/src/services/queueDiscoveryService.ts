...
export async function discoverQueues(dbId: string): Promise<DiscoveredQueue[]> {
  ...
  const skipped: { queue: string; reason: string }[] = [];
  ...
  const allQueues = rows.map((row: any) => `${row.OWNER}.${row.NAME} (table: ${row.QUEUE_TABLE})`);
  logger.info({ 
  ...
  if (skipped.length > 0) {
    ...
    const skippedByReason = skipped.reduce((acc: any, item: any) => {
      if (!acc[item.reason]) {
        acc[item.reason] = [];
      }
...
export async function discoverAllQueues(config: any): Promise<DiscoveredQueue[]> {
  const allQueues: DiscoveredQueue[] = [];
